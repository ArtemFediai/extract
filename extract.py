"""
Some handy functions mostly to extract something followed a given pattern
"""
...
def get_slices_idx_ext(A, n_slices, idx_of_interest):
    ...
    for i in np.arange(n_slices):
        IDX[i] = idx_of_interest[np.where(
            np.logical_and(A[idx_of_interest, 0] >= bin_min_max[0][i], A[idx_of_interest, 0] < bin_min_max[1][i]))[0]]
